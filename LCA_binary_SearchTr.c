  ...
  * lowest common ancestor is defined between two nodes v 
...
bool isInbetween (struct TreeNode* root, struct TreeNode* p, struct TreeNode* q) {
...
struct TreeNode* lowestCommonAncestor(struct TreeNode* root, struct TreeNode* p, struct TreeNode* q) {
    // Base case
    if (isInbetween(root, p, q)) {
        return root;
    }
    // Recursive case
    else {
        // If root value less, recursively call right
        if (root->val < p->val) {
